<template>
  <div class="theme-page min-h-screen py-16 sm:py-20">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <!-- Message Form -->
      <div class="max-w-2xl mx-auto relative" :ref="(el) => (cardRef = el as HTMLElement)" @mousemove="handleCardMouseMove" @mouseleave="handleCardMouseLeave">
        <div v-if="cardEffect.show" class="absolute w-40 h-40 rounded-full blur-2xl transition-all duration-75 ease-out pointer-events-none z-0" :style="{ left: cardEffect.x - 80 + 'px', top: cardEffect.y - 80 + 'px', background: 'radial-gradient(circle, rgba(34, 197, 94, 0.6) 0%, rgba(34, 197, 94, 0.3) 30%, rgba(34, 197, 94, 0.15) 60%, transparent 90%)', boxShadow: '0 0 80px rgba(34, 197, 94, 0.5), 0 0 160px rgba(34, 197, 94, 0.3)' }"></div>
        <div class="bg-white/80 dark:bg-white/5 backdrop-blur-xl rounded-3xl p-8 sm:p-10 border border-gray-200/60 dark:border-white/10 shadow-2xl mb-8">
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">发表留言</h2>
          <form @submit.prevent="submitMessage" class="space-y-6">
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">昵称 <span class="text-red-500">*</span></label>
              <input v-model="formData.name" type="text" required class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white/50 dark:bg-white/5 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all" placeholder="请输入你的昵称" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">邮箱 <span class="text-red-500">*</span></label>
              <input v-model="formData.email" type="email" required class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white/50 dark:bg-white/5 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all" placeholder="请输入你的邮箱" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">网站 (可选)</label>
              <input v-model="formData.website" type="text" inputmode="url" class="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white/50 dark:bg-white/5 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all" placeholder="例如：example.com 或 https://example.com" autocomplete="url" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">留言内容 <span class="text-red-500">*</span></label>
              <div class="relative">
                <RichTextarea v-model="formData.message" placeholder="请输入你的留言..." ref="messageRichTextareaRef" customClass="w-full px-4 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white/50 dark:bg-white/5 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all" />
                <button type="button" @click.stop="toggleEmotePicker" class="absolute right-3 bottom-3 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors z-10" title="插入表情包" ref="messageEmoteButtonRef">
            ...
            <div class="flex items-center justify-end space-x-4">
              <AppButton variant="primary" nativeType="submit" :disabled="submitting">{{ submitting ? "提交中..." : "提交留言" }}</AppButton>
            </div>
          </form>
        </div>
      </div>
      ...
      <!-- Messages List -->
      <div class="mt-12">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-6 pb-3 border-b border-gray-200 dark:border-gray-700">留言列表</h2>

        <div v-if="!loading && messages.length === 0" class="text-center py-10 px-5 text-gray-400 dark:text-gray-500 text-base">
          <p>还没有留言，成为第一个留言的人吧！</p>
        </div>

        <div v-if="messages.length > 0" class="flex flex-col gap-4" ref="messageListRef">
          <div v-for="(message, index) in messages" :key="message.id || index" :class="['bg-white/80 dark:bg-white/5 backdrop-blur-xl rounded-3xl p-6 border border-gray-200/60 dark:border-white/10 shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1)] hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15)] dark:hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.3)] hover:border-blue-400/70 dark:hover:border-blue-400/50 hover:-translate-y-0.5 transition-all', getNoteClass(index)]">
            <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-3">
              <div class="flex flex-col gap-1 flex-1">
                <div class="flex items-end gap-5">
                  <div v-if="message.avatar">
                    <img v-if="isHttpAvatar(message.avatar)" :src="message.avatar" alt="avatar" class="w-11 h-11 rounded-full object-cover" />
                    <div v-else v-html="sanitizeAvatar(message.avatar)"></div>
                  </div>
                  <h3 class="text-base font-semibold text-gray-900 dark:text-white m-0">{{ message.name }}</h3>
                </div>
                <span class="text-xs text-gray-400 dark:text-gray-500">{{ formatDate(message.createdAt) }}</span>
              </div>
              <div class="flex items-center gap-3 flex-shrink-0">
                <EmojiReaction :message-id="message.id || index" :reactions="message.reactions" />
                <a v-if="message.website" :href="getWebsiteUrl(message.website)" class="flex items-center justify-center w-8 h-8 rounded-md bg-white/50 text-gray-500 hover:bg-white/70 hover:text-gray-900 transition-all flex-shrink-0 dark:bg-white/10 dark:text-gray-400 dark:hover:bg-white/15 dark:hover:text-gray-300 self-start md:self-auto" title="访问网站" @click.prevent="handleWebsiteClick(message.website)">
            ...
            <div class="flex justify-between items-center">
              <div class="flex flex-wrap gap-2 my-3">
                <span v-if="message.os || message.browser || message.deviceType" class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-white/60 text-gray-500 border border-black/5 dark:bg-white/10 dark:text-gray-400 dark:border-white/10">
                  <component v-if="getOsIcon(message.os)" :is="getOsIcon(message.os)" class="w-3.5 h-3.5" />
                  {{ message.os || "未知OS" }}
                  <span class="opacity-60">·</span>
                  <component v-if="getBrowserIcon(message.browser)" :is="getBrowserIcon(message.browser)" class="w-3.5 h-3.5" />
                  {{ message.browser || "未知浏览器" }}
                  <template v-if="message.deviceType">
                    <span class="opacity-60">·</span>
                    <component :is="getDeviceIcon(message.deviceType)" class="w-3.5 h-3.5" />
                    {{ message.deviceType }}
                  </template>
                </span>
                <span v-if="message.location" class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-white/60 text-gray-500 border border-black/5 dark:bg-white/10 dark:text-gray-400 dark:border-white/10">
                  来源：{{ formatLocation(message.location) }}
                </span>
              </div>
              <div class="text-xs text-gray-500 dark:text-gray-400 cursor-pointer whitespace-nowrap" @click="handelClick(index)">
        ...
        <div v-if="loadingMore" class="flex flex-col items-center justify-center py-8 px-5">
          <Loading />
        </div>
...
const {
  ...
  fetcher: async (page, pageSize) => {
    const res = await request.get("/messages", { params: { page, pageSize, status: "approved" } });
    return {
...
const submitMessage = async () => {
  ...
  try {
    ...
    visitorStore.setInfo({ name: formData.value.name, email: formData.value.email, website: formData.value.website });
    formData.value.message = "";
    ElMessage.success("提交成功，待审核通过后展示");
    refresh();
  } catch (error) {
    console.error("提交留言失败:", error);
    const msg = (error as any)?.response?.data?.message || "提交失败，请稍后再试";
    ElMessage.error(msg);
  } finally {
...
const osIconMap: Record<string, any> = { macos: Apple, mac: Apple, windows: Laptop, win: Laptop, linux: Monitor };
const browserIconMap: Record<string, any> = { chrome: Chrome, safari: Compass, edge: Compass, firefox: Compass };
const deviceIconMap: Record<string, any> = { desktop: Monitor, laptop: Laptop, mobile: Smartphone, tablet: Smartphone };

const getOsIcon = (os?: string) => (os ? osIconMap[os.toLowerCase()] || Globe : null);
const getBrowserIcon = (browser?: string) => (browser ? browserIconMap[browser.toLowerCase()] || Globe : null);
const getDeviceIcon = (device?: string) => (device ? deviceIconMap[device.toLowerCase()] || Monitor : null);
...
const cardEffect = reactive<{ x: number; y: number; show: boolean }>({ x: 0, y: 0, show: false });
...
const handleCardMouseLeave = () => { cardEffect.show = false; };

const isHttpAvatar = (avatar: string) => typeof avatar === "string" && avatar.startsWith("http");
