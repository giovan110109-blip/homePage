...
class PhotoController {
  ...
  async upload(ctx) {
    try {
      ...
      const isImage = mimeType.startsWith('image/') || imageExts.includes(extLower)
      const isVideo = mimeType.startsWith('video/') || videoExts.includes(extLower)
      ...
      ctx.body = Response.error(`上传失败: ${error.message}`, HttpStatus.INTERNAL_ERROR)
    }
  }
  ...
  async retryTask(ctx) {
    try {
      ...
      ctx.body = Response.success({ taskId: task.taskId, status: task.status }, '已重试')
    } catch (error) {
      ctx.body = Response.error(error.message, HttpStatus.INTERNAL_ERROR)
    }
  }
  ...
  async getMapData(ctx) {
    try {
      const photos = await Photo.find({
        ...
        .select('_id title thumbnailUrl originalUrl location geoinfo dateTaken width height thumbHash')
        .sort({ dateTaken: -1 })
